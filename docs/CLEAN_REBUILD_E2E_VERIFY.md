...
- **Dashboard**: Build chỉ qua containerd (không cần npm/Node.js trên host). Chạy `./scripts/build-and-load-containerd.sh` hoặc `./scripts/build-dashboard-containerd.sh`; npm chỉ chạy trong Dockerfile (trong container).
...
- **scripts/test-priority1-apis.sh**: Chạy xong, tất cả test **PASS**:
