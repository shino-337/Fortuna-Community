...
Use `latest`, `sha-<12-char-commit>`, a pushed release tag such as `v1.5.0`, or a manual workflow `version` value as `FORTUNA_VERSION`. Tags generated by local scripts, for example `git describe` values written into `deploy/*.yaml`, are local containerd tags unless the publish workflow was run with the same value.
