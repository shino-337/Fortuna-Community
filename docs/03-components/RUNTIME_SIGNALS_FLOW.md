...
1. **Clean + Rebuild + Redeploy:**  
   - `scripts/pipeline/full-clean-database-rebuild-deploy.sh` [--skip-clean] [--skip-rebuild] [--skip-deploy] [--db] [--db-reset]  
   - Hoặc `scripts/pipeline/clean-rebuild-redeploy-and-test.sh` (gồm clean/rebuild/deploy + test, trong đó có **test-runtime-signals-e2e**).
2. **Sau khi Core/Agent chạy:**
   - **Test E2E Runtime Signals:** `./scripts/e2e/test-runtime-signals-e2e.sh`  
     (POST /runtime-events → kiểm tra DB runtime_events/runtime_signals → GET /runtime-signals và /runtime-signals/pods/:uid)
   - **Monitor luồng:** `./scripts/monitor/monitor-runtime-signals.sh` (snapshot DB + API + log Agent/Core); `./scripts/monitor/monitor-runtime-signals.sh --follow` để tail log.
