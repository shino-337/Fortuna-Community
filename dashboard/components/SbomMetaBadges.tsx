...
export function SbomMetaBadges({ sbomSource, confidence, goVersion, className = '', compact }: SbomMetaBadgesProps) {
  ...
  const sz = compact ? 'text-caption px-1.5 py-0.5' : 'text-caption px-2 py-0.5';
    ...
    <div className={`flex flex-wrap items-center gap-2 ${className}`}>
      {src === 'distroless-heuristic' && (
        <span
          ...
          title="SBOM inferred from image ref/labels (no package DB). CVE matching uses OSV-backed data in Core."
        >
      ...
      {src && src !== 'distroless-heuristic' && src !== 'label-metadata' && src !== 'parsers' && (
        <span
          className={`rounded font-mono bg-surface-2 text-text border border-border ${sz}`}
          title="SBOM source"
        >
          {src}
        </span>
      ...
      {go && (
        <span className="font-mono text-muted text-caption" title="Go toolchain (stdlib/CVE context)">
          Go: {go}
        </span>
      )}
    </div>
