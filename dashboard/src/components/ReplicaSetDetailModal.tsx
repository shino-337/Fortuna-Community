...
import { replicasetsApi, type ReplicaSet } from '../services/api'
import { formatDistanceToNow } from 'date-fns'

interface ReplicaSetDetailModalProps {
  replicaSetId: string
  onClose: () => void
}

const ReplicaSetDetailModal: React.FC<ReplicaSetDetailModalProps> = ({ replicaSetId, onClose }) => {
  const { data: replicaSetResponse, isLoading, error } = useQuery({
    queryKey: ['replicaset', replicaSetId],
    queryFn: () => replicasetsApi.getById(replicaSetId),
    enabled: !!replicaSetId,
  })

  const replicaset: ReplicaSet | undefined = replicaSetResponse?.data
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={onClose}>
        <div className="bg-white dark:bg-gray-800 rounded-lg p-6" onClick={(e) => e.stopPropagation()}>
          <div className="text-center text-red-500">
            Error loading replicaset: {error?.message || 'Not found'}
          </div>
          <button
            onClick={onClose}
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="sticky top-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4 flex items-center justify-between z-10">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{replicaset.name}</h2>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              {replicaset.namespace} • {replicaset.clusterId}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-4 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <p className="text-base text-gray-900 dark:text-white mt-1">
                  {formatDistanceToNow(new Date(replicaset.createdAt), { addSuffix: true })}
                </p>
              </div>
              <div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              <div className="text-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Replicas</p>
                <p className="text-xl font-bold text-gray-900 dark:text-white">{replicaset.replicas}</p>
              </div>
              <div className="text-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Ready</p>
                <div className="flex items-center justify-center gap-2">
                  <span className={`h-2 w-2 rounded-full ${getReplicaStatusColor(replicaset.readyReplicas, replicaset.replicas)}`}></span>
                  <p className="text-xl font-bold text-gray-900 dark:text-white">{replicaset.readyReplicas}</p>
                </div>
              </div>
              <div className="text-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Available</p>
                <p className="text-xl font-bold text-gray-900 dark:text-white">{replicaset.availableReplicas}</p>
              </div>
              <div className="text-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Fully Labeled</p>
                <p className="text-xl font-bold text-gray-900 dark:text-white">{replicaset.fullyLabeledReplicas}</p>
              </div>
            </div>
          </div>

          {/* Containers */}
          {replicaset.containers && replicaset.containers.length > 0 && (
            <div>
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">
                Containers ({replicaset.containers.length})
              </h3>
              <div className="space-y-3">
                {replicaset.containers.map((container: any, idx: number) => (
          ...
          {/* Conditions */}
          {replicaset.conditions && replicaset.conditions.length > 0 && (
            <div>
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">Conditions</h3>
              <div className="space-y-2">
                {replicaset.conditions.map((condition: any, idx: number) => (
          ...
          {/* Selector */}
          {replicaset.selector && Object.keys(replicaset.selector).length > 0 && (
            <div>
          ...
          {/* Labels */}
          {replicaset.labels && Object.keys(replicaset.labels).length > 0 && (
            <div>
          ...
          {/* Annotations */}
          {replicaset.annotations && Object.keys(replicaset.annotations).length > 0 && (
            <div>
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">Annotations</h3>
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {Object.entries(replicaset.annotations).map(([key, value]) => (
...
export default ReplicaSetDetailModal
