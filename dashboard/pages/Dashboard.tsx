...
import { Server, ShieldAlert, Boxes, Radio, ArrowRight, Shield, AlertTriangle, Bell, Info, Target, Zap } from 'lucide-react';
import { Cluster, Insight, InsightsSummary, Notification, PodCapabilitySummaryCapability, PodCapabilityTrendPoint, UnifiedRiskScore } from '../types';
...
export const Dashboard: React.FC = () => {
  ...
  // Phase 3.2: Unified Risk Pipeline additions
  const [topRiskyPods, setTopRiskyPods] = useState<UnifiedRiskScore[]>([]);
  const [exploitedCapCount, setExploitedCapCount] = useState<number>(0);
  const [attackPathCount, setAttackPathCount] = useState<number>(0);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Phase 3.2: load unified risk pipeline data (partial failure OK)
      const [topPodsResult, pipelineResult] = await Promise.allSettled([
        api.getTopRiskyPods(5),
      ...
      if (pipelineResult.status === 'fulfilled' && pipelineResult.value) {
        setExploitedCapCount(pipelineResult.value.layer2.exploitedCapCount ?? 0);
        setAttackPathCount(pipelineResult.value.layer3.totalPaths ?? 0);
      }
    } catch (e) {
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8">
        ...
        <div className="space-y-6">
                    ...
                    <div className="space-y-3">
                        {exploitedCapCount > 0 && (
                          <div className="flex items-center gap-2 text-xs text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2 mb-2">
                            ...
                            <span><strong>{exploitedCapCount}</strong> {exploitedCapCount === 1 ? 'capability' : 'capabilities'} in exploited state</span>
                            <button type="button" onClick={() => navigate('/capabilities')} className="ml-auto text-red-400 hover:text-red-300 underline text-[10px]">View</button>
                          </div>
                        )}
                        {attackPathCount > 0 && (
                          <div className="flex items-center gap-2 text-xs text-orange-400 bg-orange-500/10 border border-orange-500/20 rounded-lg px-3 py-2 mb-2">
                            <Target className="w-3 h-3 shrink-0" />
                            <span><strong>{attackPathCount}</strong> attack {attackPathCount === 1 ? 'path' : 'paths'} detected</span>
                            <button type="button" onClick={() => navigate('/attack-paths')} className="ml-auto text-orange-400 hover:text-orange-300 underline text-[10px]">View</button>
                          </div>
                        )}
                        {pceSummary.map((row) => (
            ...
            {/* Phase 3.2: Top Risky Pods card (Unified Scorer V3) */}
            {topRiskyPods.length > 0 && (
              <>
                <h3 className="text-sm font-bold text-slate-300 mt-6 flex items-center gap-2">
                  <Shield className="w-4 h-4 text-red-400" /> Top Risky Pods
                  <span className="text-[10px] text-slate-500 font-normal ml-1">unified score V3</span>
                </h3>
                <Card variant="secondary" className="p-3">
                  <div className="space-y-2">
                    {topRiskyPods.map((pod) => (
                      <div key={pod.resourceUid} className="flex items-center justify-between p-2 rounded-lg bg-slate-900/50 hover:bg-slate-800/50 cursor-pointer transition-colors" onClick={() => navigate(`/resources/${pod.resourceUid}`)}>
                        <div className="min-w-0">
                          <div className="text-xs font-semibold text-white truncate">{pod.resourceName || pod.resourceUid}</div>
                          <div className="text-[10px] text-slate-500 truncate">{pod.namespace}</div>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                          {/* Mini dimension bar */}
                          <div className="flex gap-0.5 h-3">
                            {[
                              { v: pod.dimensions?.vulnerability ?? 0, max: 15, color: '#ef4444' },
                              { v: pod.dimensions?.capabilityExposure ?? 0, max: 15, color: '#f97316' },
                              { v: pod.dimensions?.attackPath ?? 0, max: 15, color: '#eab308' },
                              { v: pod.dimensions?.rbacPolicy ?? 0, max: 15, color: '#8b5cf6' },
                            ].map((dim, i) => (
                              <div key={i} className="w-1 rounded-sm bg-slate-700 relative overflow-hidden">
                                <div className="absolute bottom-0 w-full rounded-sm" style={{ height: `${(dim.v / dim.max) * 100}%`, backgroundColor: dim.color }} />
                              </div>
                            ))}
                          </div>
                          <span className={`text-xs font-bold px-1.5 py-0.5 rounded ${
                            pod.priorityLevel === 'P0' ? 'text-red-400 bg-red-500/10' :
                            pod.priorityLevel === 'P1' ? 'text-orange-400 bg-orange-500/10' :
                            pod.priorityLevel === 'P2' ? 'text-yellow-400 bg-yellow-500/10' :
                            'text-slate-400 bg-slate-800'
                          }`}>{pod.priorityLevel}</span>
                          <span className="text-xs text-slate-300 font-mono w-8 text-right">{Math.round(pod.totalScore)}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                  <button type="button" onClick={() => navigate('/risks')} className="mt-2 text-[11px] text-pink-500 hover:text-pink-400 flex items-center gap-1">
                    View risk center <ArrowRight size={10} />
                  </button>
                </Card>
              </>
            )}
