...
kubectl delete deployment -n "$NAMESPACE" fortuna-core --ignore-not-found=true

# Delete services
kubectl delete service -n "$NAMESPACE" fortuna-core --ignore-not-found=true
...
# Step 3b: Application and bundled PostgreSQL secrets
echo ""
echo -e "${BLUE}Step 3b: Ensuring application secrets...${NC}"
if [ -x "$SCRIPTS/utils/ensure-fortuna-secrets.sh" ]; then
    if ! "$SCRIPTS/utils/ensure-fortuna-secrets.sh" "$NAMESPACE"; then
        echo -e "${RED}❌${NC} fortuna-secrets could not be applied"
        exit 1
    fi
    echo -e "${GREEN}✅${NC} fortuna-secrets ready"
else
    echo -e "${RED}❌${NC} ensure-fortuna-secrets.sh missing at $SCRIPTS/utils/ensure-fortuna-secrets.sh"
    exit 1
fi
...
if kubectl wait --for=condition=ready pod -n "$NAMESPACE" -l app=postgres --timeout=300s; then
    ...
    kubectl describe pod -n "$NAMESPACE" -l app=postgres 2>/dev/null | tail -50 || true
    if [ "${RELAX_INFRA_WAIT:-0}" = "1" ]; then
        echo -e "${YELLOW}⚠️${NC} RELAX_INFRA_WAIT=1 — continuing (Core may retry until DB is up)"
    else
        exit 1
    fi
fi
echo "Verifying PostgreSQL accepts SQL connections..."
PG_READY=false
for i in {1..30}; do
    PG_POD=$(kubectl get pod -n "$NAMESPACE" -l app=postgres -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || true)
    if [ -n "$PG_POD" ] && kubectl exec -n "$NAMESPACE" "$PG_POD" -- psql -U postgres -d fortuna -tAc "SELECT 1" 2>/dev/null | grep -qx "1"; then
        PG_READY=true
        break
    fi
    sleep 2
...
if [ "$PG_READY" = true ]; then
    echo -e "${GREEN}✅${NC} PostgreSQL SQL check passed"
elif [ "${RELAX_INFRA_WAIT:-0}" = "1" ]; then
    echo -e "${YELLOW}⚠️${NC} RELAX_INFRA_WAIT=1 — continuing although PostgreSQL SQL check failed"
else
    echo -e "${RED}❌${NC} PostgreSQL pod is Ready but SQL check failed"
    exit 1
fi
...
if kubectl wait --for=condition=ready pod -n "$NAMESPACE" -l app=nats --timeout=300s; then
    ...
    kubectl get pods -n "$NAMESPACE" -l app=nats -o wide 2>/dev/null || true
    kubectl describe pod -n "$NAMESPACE" -l app=nats 2>/dev/null | tail -50 || true
    if [ "${RELAX_INFRA_WAIT:-0}" = "1" ]; then
        echo -e "${YELLOW}⚠️${NC} RELAX_INFRA_WAIT=1 — continuing"
    else
        exit 1
    fi
fi
...
if deploy_uses_registry_images; then
    ...
    if ctr -n k8s.io images ls 2>/dev/null | grep -qE '(docker.io/library/)?fortuna-core:'; then
        ...
        if [ -x "$SCRIPTS/build/build-and-load-containerd.sh" ]; then
            echo "Running build-and-load-containerd.sh (local containerd k8s.io)..."
            ( cd "$PROJECT_ROOT" && CONTAINERD_NAMESPACE=k8s.io DOCKER_BUILDKIT="${DOCKER_BUILDKIT:-1}" "$SCRIPTS/build/build-and-load-containerd.sh" ) || {
                echo -e "${RED}❌${NC} build-and-load-containerd.sh failed; fix build errors and re-run deploy"
                exit 1
            }
        else
...
if deploy_uses_registry_images; then
    ...
    if [ -n "${VERSION:-}" ]; then
        push_cmd=(env CORE_IMAGE="fortuna-core:${VERSION}" AGENT_IMAGE="fortuna-agent:${VERSION}" DASHBOARD_IMAGE="fortuna-dashboard:${VERSION}" bash "$SCRIPTS/utils/push-images-to-workers.sh" --build-if-missing "$push_dashboard_arg")
    else
        push_cmd=(bash "$SCRIPTS/utils/push-images-to-workers.sh" --build-if-missing "$push_dashboard_arg")
    fi
    if "${push_cmd[@]}" 2>&1; then
        echo -e "${GREEN}✅${NC} Runtime images pushed to all nodes"
    else
        echo -e "${YELLOW}⚠️${NC}  Push to nodes failed (SSH or keys). Add scripts/utils/push-images.config (see push-images.config.example) or set SSH_USER/SSH_PASS; then run: $SCRIPTS/utils/push-images-to-workers.sh"
    fi
fi
...
# Step 7c2: Re-check application secrets before Core
echo ""
echo -e "${BLUE}Step 7c2: Ensuring fortuna-secrets (Core bootstrap env)...${NC}"
if [ -x "$SCRIPTS/utils/ensure-fortuna-secrets.sh" ]; then
    if ! "$SCRIPTS/utils/ensure-fortuna-secrets.sh" "$NAMESPACE"; then
        echo -e "${RED}❌${NC} fortuna-secrets could not be applied"
        exit 1
...
# Step 7d: Prerequisites check before Core/Agent (fail fast)
echo ""
...
# Step 7f: DNS pre-flight (fail fast before Core applies)
echo ""
if [ "${SKIP_DNS_PREREQ:-0}" = "1" ]; then
    echo -e "${YELLOW}⚠️${NC}  SKIP_DNS_PREREQ=1 — skipping check-dns-prereq.sh"
    ...
    echo -e "${YELLOW}⚠️${NC}  check-dns-prereq.sh not found; continuing without DNS pre-flight"
fi
...
if [ "$USE_DNS" = "false" ] && [ "$USE_IP_FALLBACK" = "true" ]; then
    echo "Configuring DATABASE_URL with IP fallback from FORTUNA_DATABASE_URL..."
    if [ -z "${FORTUNA_DATABASE_URL:-}" ]; then
        echo -e "${RED}❌${NC} USE_IP_FALLBACK=true requires FORTUNA_DATABASE_URL with the target DB host/IP."
        exit 1
    fi
    "${PROJECT_ROOT}/scripts/utils/ensure-fortuna-secrets.sh" "$NAMESPACE"
    echo -e "${GREEN}✅${NC} DATABASE_URL updated in fortuna-secrets"
fi
...
if [ "$AUTO_LOAD_CVE_ON_DEPLOY" = "true" ] && [ -x "$SCRIPTS/utils/load-cve-data.sh" ]; then
    if NAMESPACE="$NAMESPACE" PROJECT_ROOT="$PROJECT_ROOT" \
       ...
       FORCE_CVE_RELOAD="${FORCE_CVE_RELOAD:-false}" \
...
# Step 9a: Optional runtime security (Falco + eBPF)
# Controlled by env: WITH_FALCO=true and/or WITH_EBPF=true (set by pipeline script or manually).
DEPLOY_FALCO="${WITH_FALCO:-false}"
DEPLOY_EBPF="${WITH_EBPF:-false}"
if [ "$DEPLOY_FALCO" = true ]; then
  echo ""
  echo -e "${BLUE}Step 9a: Deploying Falco (Helm) for runtime security events...${NC}"
  if [ -x "$SCRIPTS/deploy/install-falco-fortuna.sh" ]; then
    if FORTUNA_NAMESPACE="$NAMESPACE" "$SCRIPTS/deploy/install-falco-fortuna.sh"; then
      ...
      echo "Waiting for Falco pods (max 120s)..."
      kubectl wait --for=condition=ready pod -l app.kubernetes.io/name=falco -n "$NAMESPACE" --timeout=120s 2>/dev/null || \
        echo -e "${YELLOW}⚠️${NC}  Falco pods not ready yet"
      kubectl set env daemonset/fortuna-agent -n "$NAMESPACE" \
        FALCO_EVENTS_ENABLED=true \
        FALCO_EVENTS_PATH="${FALCO_EVENTS_PATH:-/var/log/falco/events.jsonl}" \
        FALCO_EVENTS_POLL="${FALCO_EVENTS_POLL:-5s}" 2>/dev/null || true
    else
      echo -e "${YELLOW}⚠️${NC}  Falco install failed (helm not found or chart error)"
    fi
  else
    echo -e "${YELLOW}⚠️${NC}  install-falco-fortuna.sh not found; skipping Falco"
  fi
fi
if [ "$DEPLOY_EBPF" = true ]; then
  echo ""
...
echo ""
if [ "${WITH_FALCO:-false}" = true ]; then
  echo ""
  echo "=== Falco Status ==="
  kubectl get pods -n "$NAMESPACE" -l app.kubernetes.io/name=falco 2>/dev/null || echo "(Falco not found)"
fi
if [ "${WITH_EBPF:-false}" = true ]; then
  echo ""
  echo "=== Agent eBPF Status ==="
  echo "EBPF_ENABLED=true (set via kubectl set env)"
fi
