...
restart_flannel() {
    ...
    if kubectl rollout restart daemonset kube-flannel-ds -n kube-flannel; then
        ...
        local ready_count=$(kubectl get pods -n kube-flannel -l app=flannel --field-selector=status.phase=Running --no-headers 2>/dev/null | wc -l)
        local total_count=$(kubectl get pods -n kube-flannel -l app=flannel --no-headers 2>/dev/null | wc -l)
        
        if [ "$ready_count" -eq "$total_count" ] && [ "$total_count" -gt 0 ]; then
            log_success "Flannel pods restarted and ready ($ready_count/$total_count)"
        else
