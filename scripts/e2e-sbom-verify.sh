...
# ============================================================================
# Usage:
#   ./scripts/e2e-sbom-verify.sh                    # Check pod-test in API
#   ./scripts/e2e-sbom-verify.sh my-pod default   # Check my-pod in default ns
#   ./scripts/e2e-sbom-verify.sh --full            # Run full test-sbom-pod-flow
# ============================================================================
...
main() {
  ...
  if [ "${1:-}" = "--full" ]; then
    ...
    exec "$SCRIPT_DIR/test-sbom-pod-flow.sh" "$@"
  fi
  ...
  UID=$(get_pod_uid "$POD_NAME" "$POD_NS")
  if [ -z "$UID" ]; then
