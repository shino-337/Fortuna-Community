...
if [ -d "${CVE_DATA_DIR:-$PROJECT_ROOT/cve-data}/all" ] && [ -x "$SCRIPTS/utils/load-cve-data.sh" ]; then
  log_info "Step 1: Loading CVE data (for vulnerability insights / Threat Velocity)..."
  if "$SCRIPTS/utils/load-cve-data.sh" 2>/dev/null; then
...
if [ -x "$SCRIPTS/e2e/e2e-dashboard-data.sh" ]; then
  if "$SCRIPTS/e2e/e2e-dashboard-data.sh" 2>/dev/null; then
...
if [ -x "$SCRIPTS/e2e/test-pce-e2e.sh" ]; then
  if "$SCRIPTS/e2e/test-pce-e2e.sh" 2>/dev/null; then
...
if [ -x "$SCRIPTS/e2e/test-sbom-pod-flow.sh" ]; then
  if "$SCRIPTS/e2e/test-sbom-pod-flow.sh" 2>/dev/null; then
...
if [ -x "$SCRIPTS/verify/verify-dashboard-apis.sh" ]; then
  "$SCRIPTS/verify/verify-dashboard-apis.sh" 2>&1 || true
else
...
echo "=========================================="
echo ""
echo "Charts (Threat Velocity, PCE Trend, Risk Findings) use:"
echo "  - Threat Velocity (Risk Trend): GET .../threat-velocity?days=7[&clusterId=...]  (vulnerability insights only; needs CVE+SBOM)"
echo "  - Risk Level Overview / Total findings: GET .../insights/summary  (all insight types)"
echo "  - PCE Trend: GET .../pod-capabilities/trends?days=7  (pod_capabilities by date)"
echo ""
echo "Note: Threat Velocity only shows vulnerability (CVE) insights. E2E does not create them directly."
echo "      To see velocity change: load CVE data, deploy vuln pod, wait for agent SBOM + CVE matcher. See docs/07-guides/DASHBOARD_CHARTS_AND_E2E.md"
echo ""
...
echo ""
echo "Monitor Core/Agent (optional): ./scripts/monitor/monitor-agent-core.sh --logs 25"
echo ""
