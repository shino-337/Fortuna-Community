...
# If Core has auth enabled: set API_USER and API_PASS.
# Defaults to admin with FORTUNA_ADMIN_PASSWORD, or the first-login bootstrap password when omitted.
# ============================================================================
...
API_PASS="${API_PASS:-${FORTUNA_ADMIN_PASSWORD:-${FORTUNA_DEFAULT_ADMIN_PASSWORD:-Fortuna_ChangeMe_123!}}}"
...
if [ "$HTTP" = "200" ]; then
  ...
  echo ""
  if [ "$HTTP" = "401" ]; then
    echo "  Tip: Set API_USER and API_PASS (e.g. export API_USER=admin API_PASS=<api-password>)"
  fi
fi
echo ""
