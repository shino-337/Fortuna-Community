...
LOGIN=$(curl -sS -X POST "$API/auth/login" -H "Content-Type: application/json" \
  -d "{\"username\":\"${FORTUNA_ADMIN_USER:-admin}\",\"password\":\"${FORTUNA_ADMIN_PASSWORD:-${FORTUNA_DEFAULT_ADMIN_PASSWORD:-Fortuna_ChangeMe_123!}}\"}" 2>/dev/null) || true
TOKEN=$(echo "$LOGIN" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('token','') or '')" 2>/dev/null) || true
if [ -z "$TOKEN" ]; then
  echo "FAIL: Không lấy được JWT (Core? admin password/bootstrap default?)"
