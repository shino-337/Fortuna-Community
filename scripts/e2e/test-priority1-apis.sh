...
# JWT: login and get Bearer token (env FORTUNA_E2E_USER / FORTUNA_E2E_PASSWORD, default admin with FORTUNA_ADMIN_PASSWORD or bootstrap default). Core image has curl, not wget.
...
PG_POD=$(kubectl -n fortuna get pods -l app=postgres -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || echo "")
POD_UID=$([ -n "$PG_POD" ] && kubectl -n fortuna exec "$PG_POD" -- psql -U postgres -d fortuna -t -c "SELECT pod_uid FROM runtime_signals LIMIT 1;" 2>/dev/null | tr -d ' ' || echo "")
if [ -n "$POD_UID" ]; then
