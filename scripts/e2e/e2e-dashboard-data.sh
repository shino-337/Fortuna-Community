...
# Wait for pod to appear in Core /pods (agent sync every 5min) so PCE can run
WAIT_MAX="${E2E_POD_SYNC_WAIT:-180}"
log_info "Step 2: Waiting for pod in Core /pods (agent sync, up to ${WAIT_MAX}s)..."
WAIT_POD=0
while [ $WAIT_POD -lt "$WAIT_MAX" ]; do
  ...
  sleep 10
  WAIT_POD=$((WAIT_POD + 10))
done
if [ $WAIT_POD -ge "$WAIT_MAX" ]; then
  log_warn "Pod not seen in Core /pods after ${WAIT_MAX}s; PCE may not have run yet (agent sync interval ~5min)"
fi
echo ""
...
# --- Step 6: Verify dashboard APIs (threat-velocity = vulnerability only; no clusterId = all clusters) ---
...
# Verify threat-velocity with clusterId (same as Risk Center when cluster selected)
CLUSTERS_JSON=$(api_get "clusters" 2>/dev/null || echo "{}")
FIRST_CLUSTER_ID=$(echo "$CLUSTERS_JSON" | python3 -c "import sys,json; d=json.load(sys.stdin); c=d.get('clusters',[]); print(c[0].get('id','') if c else '')" 2>/dev/null || echo "")
if [ -n "$FIRST_CLUSTER_ID" ]; then
  TV_SCOPED=$(api_get "dashboard/metrics/threat-velocity?days=7&clusterId=$FIRST_CLUSTER_ID" 2>/dev/null || echo "{}")
  TV_SCOPED_SUM=$(echo "$TV_SCOPED" | python3 -c "
import sys,json
d=json.load(sys.stdin)
t=d.get('trend',[])
s=0
for x in t: s+=(x.get('critical') or 0)+(x.get('high') or 0)+(x.get('medium') or 0)+(x.get('low') or 0)
print(s)
" 2>/dev/null || echo "0")
  log_ok "Threat Velocity (clusterId=$FIRST_CLUSTER_ID): total in window=$TV_SCOPED_SUM (Risk Center uses this when cluster selected)"
else
  log_warn "No cluster id for threat-velocity scoped check (Risk Center cluster filter)"
fi
echo ""
...
echo "Refresh dashboard with the configured admin credentials to see data."
echo ""
