...
#   Env: FORTUNA_PACKAGE_SOURCE=github|local controls workload image source. Default github uses GHCR packages.
#        Set FORTUNA_PACKAGE_SOURCE=local for registryless source builds.
...
_sync_deploy_image_tags() {
  ...
  _image_ref() {
    ...
    case "$FORTUNA_PACKAGE_SOURCE" in
      github|ghcr|registry)
        echo "${FORTUNA_REGISTRY%/}/${name}:${FORTUNA_VERSION}"
        ;;
      local)
        echo "${name}:${VERSION}"
        ;;
  ...
  _sync_manifest_image() {
    local file="$1"
    local name="$2"
    local ref
    ref="$(_image_ref "$name")"
    [ -f "$file" ] || return 0
    sed -i -E "s@image:[[:space:]]*[^[:space:]#]*/?${name}:[^[:space:]#]+@image: ${ref}@g" "$file"
  ...
  case "${COMPONENT_ONLY:-}" in
    core)
      ...
      ;;
    agent)
      _sync_manifest_image "$PROJECT_ROOT/deploy/fortuna-agent-daemonset.yaml" "fortuna-agent"
      ;;
    dashboard)
      _sync_manifest_image "$PROJECT_ROOT/deploy/dashboard-deployment.yaml" "fortuna-dashboard"
      ;;
    *)
      _sync_manifest_image "$PROJECT_ROOT/deploy/fortuna-core-deployment.yaml" "fortuna-core"
      _sync_manifest_image "$PROJECT_ROOT/deploy/fortuna-agent-daemonset.yaml" "fortuna-agent"
      [ "${SKIP_DASHBOARD:-false}" = "true" ] || _sync_manifest_image "$PROJECT_ROOT/deploy/dashboard-deployment.yaml" "fortuna-dashboard"
      ;;
  esac
}
...
_set_workload_images_for_tag() {
  ...
  if [ "$FORTUNA_PACKAGE_SOURCE" = "local" ]; then
    ...
    core_image="${FORTUNA_REGISTRY%/}/fortuna-core:${FORTUNA_VERSION}"
    agent_image="${FORTUNA_REGISTRY%/}/fortuna-agent:${FORTUNA_VERSION}"
    dashboard_image="${FORTUNA_REGISTRY%/}/fortuna-dashboard:${FORTUNA_VERSION}"
  fi
  case "${COMPONENT_ONLY:-}" in
    core)
...
if [ "$FORTUNA_PACKAGE_SOURCE" != "local" ]; then
  log_info "Phase 2b skipped: workloads use GitHub/GHCR packages (${FORTUNA_REGISTRY%/}:$FORTUNA_VERSION)."
elif [ "$SKIP_DEPLOY" = false ] && [ "$SKIP_REBUILD" = false ] && [ "$PUSH_IMAGES_AFTER_REBUILD" = true ]; then
