...
# Usage: ./scripts/verify/verify-dashboard-api.sh
...
# Login
echo "[1] Login..."
LOGIN=$(curl -s -X POST "$API/auth/login" -H "Content-Type: application/json" -d "{\"username\":\"${FORTUNA_ADMIN_USER:-admin}\",\"password\":\"${FORTUNA_ADMIN_PASSWORD:-${FORTUNA_DEFAULT_ADMIN_PASSWORD:-Fortuna_ChangeMe_123!}}\"}" 2>/dev/null) || true
...
echo "  1. You are logged in with the configured admin credentials"
