...
section_api() {
  ...
  TOKEN=$(kubectl exec -n "$NAMESPACE" "$CORE_POD" -- curl -s -X POST http://localhost:8080/api/v1/auth/login \
    -H 'Content-Type: application/json' -d "{\"username\":\"${FORTUNA_ADMIN_USER:-admin}\",\"password\":\"${FORTUNA_ADMIN_PASSWORD:-${FORTUNA_DEFAULT_ADMIN_PASSWORD:-Fortuna_ChangeMe_123!}}\"}" 2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin).get('token','') or '')" 2>/dev/null || true)
  if [ -z "$TOKEN" ]; then
