...
if [ -z "$TOKEN" ]; then
  echo "ERROR: Could not get JWT (login failed). Check admin/admin123 or Core auth."
  exit 1
fi
...
echo "Done. If all show OK/counts, Dashboard should display data when logged in (admin / admin123)."
