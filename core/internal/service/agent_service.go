	...
	for _, saData := range sas {
		...
		if err == nil {
			...
			changed := existingSA.Name != sa.Name ||
				...
				!equalJSON(existingSA.Labels, sa.Labels) ||
				!equalJSON(existingSA.Secrets, sa.Secrets) ||
				!equalJSON(existingSA.LinkedPods, sa.LinkedPods)
	...
	for _, roleData := range roles {
		...
		if err == nil {
			...
			changed := existing.Name != role.Name ||
				...
				!equalJSON(existing.Rules, role.Rules)
	...
	for _, crData := range clusterRoles {
		...
		if err == nil {
			...
			changed := existing.Name != clusterRole.Name ||
				!equalJSON(existing.Rules, clusterRole.Rules)
	...
	for _, rbData := range roleBindings {
		...
		if err == nil {
			...
			changed := existing.Name != roleBinding.Name ||
				...
				!equalJSON(existing.RoleRef, roleBinding.RoleRef) ||
				!equalJSON(existing.Subjects, roleBinding.Subjects)
	...
	for _, crbData := range clusterRoleBindings {
		...
		if err == nil {
			...
			changed := existing.Name != clusterRoleBinding.Name ||
				!equalJSON(existing.RoleRef, clusterRoleBinding.RoleRef) ||
				!equalJSON(existing.Subjects, clusterRoleBinding.Subjects)
