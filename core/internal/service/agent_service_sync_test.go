...
func TestProcessSyncedPods_PartialPayloadNoMassDelete(t *testing.T) {
	db := openTestDB(t)
	if err := db.AutoMigrate(&models.Cluster{}, &models.Pod{}, &models.PodInstance{}); err != nil {
		t.Fatalf("migrate: %v", err)
	}
	clusterID := "sync-partial-pods"
	if err := db.Create(&models.Cluster{ID: clusterID, Name: "test"}).Error; err != nil {
		t.Fatalf("create cluster: %v", err)
	}
	for i := 1; i <= 5; i++ {
	...
	if err := svc.processSyncedPods(clusterID, data, true); err != nil {
		t.Fatalf("processSyncedPods(partial): %v", err)
	}

	var active int64
	if err := db.Model(&models.Pod{}).Where("cluster_id = ?", clusterID).Count(&active).Error; err != nil {
		t.Fatalf("count active pods: %v", err)
	}
	if active != 5 {
		t.Fatalf("partial payload deleted pods: active=%d, want 5", active)
	}
}
