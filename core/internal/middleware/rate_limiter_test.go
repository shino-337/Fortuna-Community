...
func TestDefaultRateLimiterConfig_BurstExhaustion(t *testing.T) {
	...
	// Use a very slow refill so elapsed time during the loop cannot add tokens (avoids flake vs 100 RPS).
	l := rate.NewLimiter(rate.Every(24*time.Hour), cfg.BurstSize)
