...
func GetSupplyChainCorrelation(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		rows, err := querySupplyChain(ctx, db, namespace, severity, nodeName, limit)
		if err != nil {
...
func querySupplyChain(ctx context.Context, db *gorm.DB, namespace, severity, nodeName string, limit int) ([]supplyChainRow, error) {
...
LIMIT $4
`, namespace, severity, nodeName, limit)
