...
func governanceFixture(t *testing.T) (*gorm.DB, func(string, string, string, string) *httptest.ResponseRecorder) {
	...
	if err := db.Where("uid = ?", "a-history").Delete(&models.Pod{}).Error; err != nil {
		t.Fatal(err)
	}
	...
	if err := db.Create(&models.ExceptionPolicy{ResourceUID: "orphan", CVEID: "CVE-test", InsightType: "vulnerability"}).Error; err != nil {
		t.Fatal(err)
	}
	r := gin.New()
	...
	group.Use(func(c *gin.Context) {
		...
		if who == "viewer" {
			perms = []authorization.Permission{authorization.PermissionFindingsRead}
		}
...
func TestRiskGovernanceAggregateScope(t *testing.T) {
	...
	for _, path := range []string{"/attack-steps/summary", "/exceptions"} {
		...
		if w := request("GET", path+"?cluster=a&clusterId=b", "admin", ""); w.Code != 400 {
			t.Fatalf("conflict: %d", w.Code)
		}
		if w := request("GET", path, "missing", ""); w.Code != 401 {
	...
	if len(summary.Summary) != 1 || summary.Summary[0].Count != 1 || summary.Summary[0].AvgConfidence != .25 {
	...
	if w.Code != 200 || summary.Summary[0].Count != 2 {
		t.Fatalf("global: %s", w.Body)
	}
	var exceptions struct {
		Exceptions []models.ExceptionPolicy
		Total      int
	}
	for _, tc := range []struct {
		...
		if w.Code != 200 || exceptions.Total != tc.total {
...
func TestRiskExceptionsMutationsRespectOwnership(t *testing.T) {
	...
	body := func(uid string) string {
		return "{\"resourceUid\":\"" + uid + "\",\"cveId\":\"CVE-new\",\"insightType\":\"vulnerability\",\"reason\":\"test\"}"
	}
	...
	for _, uid := range []string{"b", "orphan"} {
		if w := request("POST", "/exceptions", "a", body(uid)); w.Code != 404 {
			t.Fatalf("create %s: %d %s", uid, w.Code, w.Body)
		}
	}
	if w := request("DELETE", "/exceptions/2", "a", ""); w.Code != 404 || strings.Contains(w.Body.String(), `"b"`) {
		t.Fatalf("foreign delete disclosed ownership: %d %s", w.Code, w.Body)
	}
	if w := request("DELETE", "/exceptions/not-a-number", "a", ""); w.Code != 400 {
		t.Fatalf("bad ID: %d", w.Code)
	}
	if w := request("POST", "/exceptions", "viewer", body("a")); w.Code != 403 {
		t.Fatalf("viewer create: %d", w.Code)
	}
	var count int64
	if err := db.Model(&models.ExceptionPolicy{}).Count(&count).Error; err != nil {
		t.Fatal(err)
	}
	if count != 4 {
		t.Fatalf("denied writes changed count: %d", count)
	}
	if w := request("POST", "/exceptions", "a", body("a-history")); w.Code != 201 {
		t.Fatalf("historical create: %d %s", w.Code, w.Body)
	}
	if w := request("DELETE", "/exceptions/3", "a", ""); w.Code != 200 {
		t.Fatalf("historical delete: %d %s", w.Code, w.Body)
	}
}
...
func TestGlobalEvaluationRejectsScopeBeforeWork(t *testing.T) {
	...
	for _, path := range []string{"/insights/evaluate", "/insights/evaluate/historical"} {
		for _, tc := range []struct {
			...
			if w.Code != tc.status {
				t.Fatalf("%s %+v: %d %s", path, tc, w.Code, w.Body)
			}
		}
	}
	if err := db.Migrator().DropTable(&models.ExceptionPolicy{}, &models.PodAttackStep{}); err != nil {
		t.Fatal(err)
	}
