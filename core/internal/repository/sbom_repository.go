...
func (r *SBOMRepository) UpsertSBOMWithComponents(
	...
	case err == nil:
		...
		// Transient pull errors (no components) must not wipe a previously-good SBOM snapshot.
		// Keep status/package_count/components from the existing row until a successful refresh arrives.
		preserveExistingSnapshot = len(components) == 0 &&
			normalizeSBOMStatus(sbom.Status) == "failed" &&
			strings.EqualFold(strings.TrimSpace(sbom.StatusReason), "pull_error") &&
		...
		if preserveExistingSnapshot {
			updateFields["package_count"] = existing.PackageCount
			updateFields["status_reason"] = existing.StatusReason
			updateFields["status"] = normalizeSBOMStatus(existing.Status)
			updateFields["normalized_fingerprint"] = existing.NormalizedFingerprint
		} else {
			updateFields["package_count"] = sbom.PackageCount
			updateFields["status_reason"] = sbom.StatusReason
			updateFields["status"] = normalizeSBOMStatus(sbom.Status)
		}
	...
	if len(components) > 0 && !preserveExistingSnapshot {
