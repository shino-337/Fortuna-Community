...
// Migration099_RefreshYAMLRiskRulesMITRE refreshes risk_rules table from core/rules YAML,
// so any newly added/updated YAML rules (including MITRE ATT&CK tags) appear immediately in DB/UI.
func Migration099_RefreshYAMLRiskRulesMITRE(db *gorm.DB) error {
	log.Println("Running migration 099: Refresh YAMLRiskRules (MITRE tags)")
	...
	// Cleanup any accidental empty rule_id row.
	_ = db.Unscoped().Where("rule_id = ''").Delete(&models.RiskRule{})

	rulesDir := resolveRulesDir()
	if rulesDir == "" {
		log.Println("[migration 099] No rules directory found. Skipping.")
		return nil
	}
	...
	rules := ye.GetRules()
	if len(rules) == 0 {
		log.Printf("[migration 099] No YAML rules loaded from %s. Skipping.\n", rulesDir)
		return nil
	}
	...
	for i := range rules {
		...
		if err != nil {
			log.Printf("[migration 099] Skip %s: RuleToRiskRule error: %v\n", r.ID, err)
			continue
		}
	...
	log.Printf("[migration 099] Upserted/Refreshed %d risk rules from %s\n", upserted, rulesDir)
	return nil
}
