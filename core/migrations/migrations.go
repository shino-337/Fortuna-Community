...
//
// Classification (see core/migrations/README.md):
...
var (
	...
	_ = Migration139_AddSBOMMatchRunCatalogGeneration
	_ = Migration140_AddCVECatalogGenerationProvenance
	_ = Migration141_AddOSVCatalogGenerationProvenance
	_ = Migration142_AddOperationalQueryIndexes
	_ = Migration143_EnsurePolicyInstanceTables
	...
	_ = Migration120_SoftDeleteLegacyRiskScoreRows
	_ = Migration121_AddRuntimeSignalStepMappings
	_ = Migration122_HardenRuntimeSignalStepMappings
	_ = Migration123_AddAuditAnchor
	_ = Migration124_RiskScoresV3ScorerComment
	_ = Migration125_BackfillInsightCVEIDRuleIDs
	_ = Migration127_AddUsersLastLoginIfMissing
	_ = Migration128_RBACV2ActivityAndUserScope
	_ = Migration129_SecurityActivityAuditV2
	_ = Migration130_UserSessions
	_ = Migration131_ClusterSensitivity
	_ = Migration132_InsightSensitivity
	_ = Migration133_RepairPolicyEngineAfterPartialReset
	_ = Migration134_InvestigationCases
	_ = Migration135_InvestigationTimelineV2
	_ = Migration147_UserPasswordBootstrapState
...
// Each row records a migration index that has been applied successfully.
func ensureSchemaMigrationsTable(db *gorm.DB) error {
	return execDDL(db, `CREATE TABLE IF NOT EXISTS schema_migrations (
		version INTEGER PRIMARY KEY,
		applied_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW()
	)`)
}

// migrationApplied returns true if version has already been recorded.
func migrationApplied(db *gorm.DB, version int) bool {
	var exists bool
	if err := db.Raw("SELECT EXISTS (SELECT 1 FROM schema_migrations WHERE version = ?)", version).Scan(&exists).Error; err != nil {
		return false
	}
	return exists
}

func recordMigration(db *gorm.DB, version int) {
	_ = db.Exec("INSERT INTO schema_migrations (version) VALUES (?) ON CONFLICT DO NOTHING", version)
}

// RunMigrations runs all database migrations, skipping already-applied ones.
func RunMigrations(db *gorm.DB) error {
	log.Println("Running database migrations...")

	if err := ensureSchemaMigrationsTable(db); err != nil {
		log.Printf("WARNING: could not create schema_migrations table: %v (will run all migrations)", err)
	}
	...
	migrations := []func(*gorm.DB) error{
		...
		Migration120_SoftDeleteLegacyRiskScoreRows,              // V3 cutover: soft-delete legacy v1/v2 risk_scores rows (upsert revives)
		Migration121_AddRuntimeSignalStepMappings,               // Configurable runtime signal -> attack step mappings for progress reasoning
		Migration122_HardenRuntimeSignalStepMappings,            // Control-plane hardening fields for runtime signal-step mappings
		Migration123_AddAuditAnchor,                             // Tamper-evident audit hash-chain anchor checkpoints
		Migration124_RiskScoresV3ScorerComment,                  // Docs: risk_scores.scorer_version + total_score reflect authoritative V3
		Migration125_BackfillInsightCVEIDRuleIDs,                // Backfill insights.cve_id with YAML rule IDs for exact-match queries
		Migration126_AddRuntimeEventsConfidence,                 // Runtime: confidence column (ingestion-required, >0 for scoring)
		Migration127_AddUsersLastLoginIfMissing,                 // users.last_login for login audit (align 002 SQL with models.User)
		Migration128_RBACV2ActivityAndUserScope,                 // RBAC v2: security_activity_logs + users.scope_json
		Migration129_SecurityActivityAuditV2,                    // Governance: audit columns + append-only triggers
		Migration130_UserSessions,                               // JWT session binding + revocation
		Migration131_ClusterSensitivity,                         // clusters.sensitivity classification
		Migration132_InsightSensitivity,                         // findings.sensitivity classification hook
		Migration133_RepairPolicyEngineAfterPartialReset,        // repair policy_* tables + baseline seed after partial SQL reset
		Migration134_InvestigationCases,                         // SOC investigation workspace cases
		Migration135_InvestigationTimelineV2,                    // Append-only timeline + lifecycle v2
		Migration136_AddMalwareFeedSyncRuns,                     // Supply-chain malware feed freshness and sync outcome
		Migration137_AddMalwareEcosystem,                        // Supply-chain malware ecosystem-scoped matching
		Migration138_AddCatalogGenerations,                      // Supply-chain catalog generation metadata
		Migration139_AddSBOMMatchRunCatalogGeneration,           // Supply-chain matcher run provenance by active catalog generation
		Migration140_AddCVECatalogGenerationProvenance,          // Supply-chain CVE row provenance by catalog generation
		Migration141_AddOSVCatalogGenerationProvenance,          // Supply-chain OSV mirror row provenance by catalog generation
		Migration142_AddOperationalQueryIndexes,                 // Operational query indexes for graph RBAC and latest V3 risk score lookup
		Migration143_EnsurePolicyInstanceTables,                 // Policy engine repair after partial schema reset
		Migration144_RepairPolicyEngineTablesSQL,                // Policy engine repair using explicit DDL after partial schema reset
		Migration145_SeedPolicyEngineBaselineSQL,                // Policy engine baseline seed using explicit SQL after partial schema reset
	...
	for i, migration := range migrations {
		ver := i + 1
		if migrationApplied(db, ver) {
			skipped++
			continue
		}
		log.Printf("Executing migration %d of %d", ver, len(migrations))
		err := migration(db)
		if err != nil {
			errStr := err.Error()
			log.Printf("Migration %d returned error: %s", ver, errStr)
			if errStr != "" && (strings.Contains(errStr, "insufficient arguments") ||
				...
				log.Printf("WARNING: Migration %d encountered known GORM/PostgreSQL issue: %s", ver, errStr)

				if i == 0 {
					requiredTables := []string{"clusters", "service_accounts", "roles", "cluster_roles",
						"role_bindings", "cluster_role_bindings", "pods", "audit_logs"}
					if validationErr := validateMigrationResult(db, ver, requiredTables); validationErr != nil {
						return fmt.Errorf("migration %d schema validation failed: %w", ver, validationErr)
					}
					log.Printf("Migration %d: All required tables validated successfully, continuing...", ver)
					recordMigration(db, ver)
					applied++
					continue
				}

				log.Printf("WARNING: Migration %d failed with known issue, but continuing with next migration", ver)
				recordMigration(db, ver)
				applied++
				continue
			}
			log.Printf("ERROR: Migration %d failed: %v", ver, err)
	...
	log.Printf("Migrations complete: %d applied, %d skipped (already applied), %d total", applied, skipped, len(migrations))
...
const DefaultBootstrapAdminPassword = "Fortuna_ChangeMe_123!"

// CreateDefaultAdmin creates a default admin user if it doesn't exist.
// Operator-provided passwords are synced for rotation; built-in bootstrap defaults are never synced over an existing account.
func CreateDefaultAdmin(db *gorm.DB, username, password, email string, forcePasswordChange bool, bootstrapCredential bool) error {
	if err := auth.ValidatePasswordStrength(password); err != nil {
		if os.Getenv("FORTUNA_ALLOW_WEAK_BOOTSTRAP_PASSWORD") != "true" {
			return fmt.Errorf("FORTUNA_ADMIN_PASSWORD does not meet password policy (%w); use a stronger password or set FORTUNA_ALLOW_WEAK_BOOTSTRAP_PASSWORD=true only for non-production", err)
		}
		log.Printf("[Security] WARNING: bootstrap admin password fails policy; allowed because FORTUNA_ALLOW_WEAK_BOOTSTRAP_PASSWORD=true")
	}
	...
	if errors.Is(result.Error, gorm.ErrRecordNotFound) {
		...
		log.Printf("Created default admin user: %s (forcePasswordChange=%t, bootstrapCredential=%t)", username, forcePasswordChange, bootstrapCredential)
		return nil
	}
	...
	if user.Password != hashedPassword {
		...
		user.BootstrapCredential = false
		...
		if user.MustChangePassword != forcePasswordChange {
			updates["must_change_password"] = forcePasswordChange
		}
		if user.BootstrapCredential != bootstrapCredential {
			updates["bootstrap_credential"] = bootstrapCredential
		}
		if len(updates) > 0 {
			if err := db.Model(&user).Updates(updates).Error; err != nil {
				return fmt.Errorf("failed to update admin password state: %w", err)
			}
		}
...
func RunPostMigrations(db *gorm.DB) error {
	...
	// Create or sync the bootstrap admin user. If no operator-provided password exists,
	// use the built-in first-login credential and force a password change.
	adminUsername := strings.TrimSpace(os.Getenv("FORTUNA_ADMIN_USERNAME"))
	if adminUsername == "" {
		adminUsername = "admin"
	}
	adminPassword := strings.TrimSpace(os.Getenv("FORTUNA_ADMIN_PASSWORD"))
	defaultAdminPassword := strings.TrimSpace(os.Getenv("FORTUNA_DEFAULT_ADMIN_PASSWORD"))
	if defaultAdminPassword == "" {
		defaultAdminPassword = DefaultBootstrapAdminPassword
	}
	bootstrapCredential := envTruthy("FORTUNA_BOOTSTRAP_DEFAULT_CREDENTIAL") || adminPassword == defaultAdminPassword
	if adminPassword == "" {
		adminPassword = defaultAdminPassword
		bootstrapCredential = true
	}
	if bootstrapCredential {
		log.Printf("[Security] WARNING: bootstrap admin is using a default first-login password; change it immediately.")
	}
	adminEmail := strings.TrimSpace(os.Getenv("FORTUNA_ADMIN_EMAIL"))
	if adminEmail == "" {
