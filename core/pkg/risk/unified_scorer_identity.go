...
func (s *UnifiedScorerV3) CalculateScoreV3ForIdentity(ctx context.Context, id resourceidentity.Identity) (*UnifiedScoreV3, error) {
	...
	if s.db.Migrator().HasTable("pod_capabilities") {
		if err := s.db.WithContext(ctx).Where("cluster_id = ? AND pod_uid = ?", id.ClusterID, id.ResourceUID).Find(&caps).Error; err != nil {
	...
	if s.db.Migrator().HasTable("attack_paths") {
		if err := s.db.WithContext(ctx).Where("cluster_id = ? AND pod_uid = ?", id.ClusterID, id.ResourceUID).Find(&attackPaths).Error; err != nil {
	...
	if s.db.Migrator().HasTable("runtime_signals") {
		...
		var sum24 struct{ Total int64 `gorm:"column:total"` }
		...
		if err := s.db.WithContext(ctx).Model(&models.RuntimeSignal{}).Distinct("signal_type").
	...
	pathInf := computePathInfluence(attackPaths, runtimeSignals, signalToSteps, baseRiskAnchor, &pathRuntimeAlign{
		Progress: runtimeAlignProgress, MatchedSignals: matchedRuntimeForPath, CriticalHitCount: runtimeCritHits,
	})
	...
	runtimeFreshness := computeDimFreshness(maxTime(newestInsightTime(insights, "runtime", "runtime_threat", "runtime-behavior"), newestRuntimeSignalTime(runtimeSignals)))
	...
	aggEngine := NewRiskAggregationEngineV3(
		map[string]float64{"vulnerability":15,"capability":15,"attack_path":15,"rbac_policy":15,"runtime":15,"exposure":15,"blast_radius":10},
		DefaultDimensionWeights, DefaultSourceConfidence, ctxInfo.EffectiveMultiplier,
	)
	identityKey := id.Key()
	...
	for _, combo := range agg.InteractionCombos { toxicCombos = append(toxicCombos, combo.Name) }
	...
	if err := s.db.WithContext(ctx).
		...
		Order("calculated_at DESC").First(&prev).Error; err == nil {
		value := prev.TotalScore
		previousScore = &value
	}
	trendDelta := 0.0
	if previousScore != nil { trendDelta = rawScore - *previousScore }
	...
	totalScore, temporal := ComputeTemporalScore(rawScore, previousScore, TemporalSignals{
		BurstEvents5m:int(runtimeBurstEvents5m), UniqueSignalTypes5m:int(runtimeUniqueSignalTypes5m), TrendDelta:trendDelta, PersistenceMinutes:persistenceMinutes,
	})
	...
	factors := map[string]interface{}{
		...
		"dimensions":map[string]interface{}{
			"vulnerability":math.Round(vulnScore*100)/100,"capability_exposure":math.Round(capScore*100)/100,
			"attack_path":math.Round(pathScore*100)/100,"rbac_policy":math.Round(rbacScore*100)/100,
			"runtime_threat":math.Round(runtimeScore*100)/100,"exposure":math.Round(exposureScore*100)/100,"blast_radius":math.Round(blastScore*100)/100,
		},
		"toxic_combos":toxicCombos,"time_decay":math.Round(timeDecay*100)/100,
		"runtime_signals_24h":runtimeSignalEvents24h,"runtime_signals_5m":runtimeUniqueSignalTypes5m,"runtime_events_5m":runtimeBurstEvents5m,
		"runtime_derived_capabilities":runtimeDerivedCaps,"runtime_satisfied_requirements":runtimeSatisfiedRequirements(runtimeEvents24h,runtimeSignals),
		"sa_token_satisfaction_weight":math.Round(saTokSatW*1000)/1000,"path_mitre_relevance_count":len(pathMitreSet),
		"runtime_temporal_coherence":map[string]interface{}{
			"sequence_bonus":temporalCoherence.SequenceBonus,"disorder_penalty":temporalCoherence.DisorderPenalty,
			"coherence_multiplier":temporalCoherence.CoherenceMultiplier,"pairing_window_exceeded":temporalCoherence.PairingWindowExceeded,
			"pairing_gap_seconds":temporalCoherence.PairingGapSeconds,
		},
		"capability_validation_confidence":math.Round(capValConf*1000)/1000,"runtime_confidence_score":math.Round(runtimeConfSc*1000)/1000,
		"drift_score_raw":math.Round(trendDelta*100)/100,"corroboration_k8s_api":corroborationK8s,
		"explainability":map[string]interface{}{"runtime_threat":map[string]interface{}{
			"matched_runtime_signals":pathInf.MatchedRuntimeSignals,"runtime_step_progress":pathInf.RuntimeStepProgress,
			"k8s_api_corroboration":corroborationK8s,"satisfied_requirements":runtimeSatisfiedRequirements(runtimeEvents24h,runtimeSignals),
		}},
		"insights_count":len(insights),"capabilities_count":len(caps),"attack_paths_count":len(attackPaths),
		"attack_path_reasoning":map[string]interface{}{
			"max_strength":pathInf.MaxStrength,"avg_strength":pathInf.AvgStrength,"progress":pathInf.Progress,"reaches_node":pathInf.ReachesNode,
			"class_counts":pathInf.ClassCounts,"e_inject_norm":pathInf.EInjectNorm,"r_inject_norm":pathInf.RInjectNorm,"i_inject_norm":pathInf.IInjectNorm,
			"graph_based_e":pathInf.GraphBasedE,"runtime_based_e":pathInf.RuntimeBasedE,"runtime_decay":pathInf.RuntimeDecay,"diversity_factor":pathInf.DiversityFactor,
			"critical_hit_count":pathInf.CriticalHitCount,"e_inject_points":pathInf.EInjectPoints,"r_inject_points":pathInf.RInjectPoints,"i_inject_points":pathInf.IInjectPoints,
			"applied_path_count":pathInf.AppliedPathCount,"runtime_step_progress":pathInf.RuntimeStepProgress,"progress_raw":pathInf.ProgressRaw,
			"progress_saturated":pathInf.ProgressSaturated,"inject_scale":pathInf.InjectScale,"matched_runtime_signals":pathInf.MatchedRuntimeSignals,
		},
		"path_influence":map[string]interface{}{
			"max_strength":pathInf.MaxStrength,"progress_raw":pathInf.ProgressRaw,"progress_saturated":pathInf.ProgressSaturated,"inject_scale":pathInf.InjectScale,
			"runtime_decay":pathInf.RuntimeDecay,"diversity_factor":pathInf.DiversityFactor,"critical_hit_count":pathInf.CriticalHitCount,
			"axis_injection":map[string]interface{}{"exploitability":pathInf.EInjectNorm,"impact":pathInf.IInjectNorm,"reachability":pathInf.RInjectNorm},
		},
		"mapping_version":mappingMeta,"temporal":temporal,
		"context":map[string]interface{}{"tier":ctxInfo.Tier,"base_multiplier":ctxInfo.BaseMultiplier,"internet_facing":ctxInfo.InternetFacing,"system_namespace":ctxInfo.SystemNamespace,"effective_multiplier":ctxInfo.EffectiveMultiplier},
		"aggregation":map[string]interface{}{
			"mode":"v3_triaxial","final_formula":agg.FinalFormula,"total_score_raw":agg.TotalScoreRaw,"base_risk":agg.BaseRisk,"threat_core":agg.ThreatCore,
			"threat_amplifier_raw":agg.ThreatAmplifierRaw,"threat_amplifier":agg.ThreatAmplifier,"combo_threat_boost":agg.ComboThreatBoost,
			"max_threat_amplifier":agg.MaxThreatAmplifier,"category_sums":agg.CategorySums,"category_caps":agg.CategoryCaps,"risk_factors":agg.Factors,
			"interaction_combos":agg.InteractionCombos,"axes":agg.Axes,"combo_amplifier":agg.ComboAmplifier,"overrides_applied":agg.OverridesApplied,"context_multiplier":agg.ContextMultiplier,
		},
	}
	for key, value := range mitreBoostMeta { factors[key] = value }
	for key, value := range runtimeThreatMeta { factors[key] = value }
	dimSum := vulnScore+capScore+pathScore+rbacScore+runtimeScore+exposureScore+blastScore
	if dimSum > 1e-6 { factors["runtime_influence_ratio_vs_dimensions"] = math.Round((runtimeScore/dimSum)*1000)/1000 }
	ratioVsTotal := 0.0
	if totalScore >= 5.0-1e-6 && totalScore > 1e-6 { ratioVsTotal = math.Round((runtimeScore/totalScore)*1000)/1000 }
	factors["runtime_influence_ratio_vs_total"] = ratioVsTotal
	if scorerCorrPrec > 0 && scorerCorrPrec < 0.35 { factors["alert_over_inference_risk"] = true }
	if ratioVsTotal > 0.42 && totalScore >= 5.0-1e-6 { factors["alert_runtime_dominance"] = true }
	if ratioVsTotal < 0.06 && len(runtimeSignals) > 4 && totalScore >= 5.0-1e-6 { factors["alert_silent_runtime"] = true }

	return &UnifiedScoreV3{
		ResourceUID:id.ResourceUID,ResourceName:resourceName,ResourceType:resourceType,Namespace:namespace,ClusterID:id.ClusterID,
		PriorityLevel:"",TotalScore:totalScore,TimeDecay:math.Round(timeDecay*100)/100,ScorerVersion:"v3",
		VulnerabilityScore:math.Round(vulnScore*100)/100,CapabilityExposureScore:math.Round(capScore*100)/100,
		AttackPathScore:math.Round(pathScore*100)/100,RBACPolicyScore:math.Round(rbacScore*100)/100,
		RuntimeThreatScore:math.Round(runtimeScore*100)/100,ExposureScore:math.Round(exposureScore*100)/100,
		BlastRadiusScore:math.Round(blastScore*100)/100,ToxicCombos:toxicCombos,Factors:factors,CalculatedAt:time.Now().UTC(),
	}, nil
}
...
func (s *UnifiedScorerV3) ScheduleUnifiedScoreCalculationForIdentity(id resourceidentity.Identity) {
	if id.Validate() != nil { return }
	go func() {
		ctx := context.Background()
		score, err := s.CalculateScoreV3ForIdentity(ctx, id)
		if err != nil {
			log.Printf("[UnifiedScorerV3] scoped calculation failed for %s: %v", id.Key(), err)
			return
		}
		if err := s.SaveScoreV3ForIdentity(ctx, id, score); err != nil {
			log.Printf("[UnifiedScorerV3] scoped save failed for %s: %v", id.Key(), err)
			return
		}
		log.Printf("[UnifiedScorerV3] scoped score updated for %s total=%.1f", id.Key(), score.TotalScore)
	}()
}
