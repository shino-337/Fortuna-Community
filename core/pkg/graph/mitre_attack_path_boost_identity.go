...
// ComputeAttackPathMitreBoostForIdentity is the multi-cluster-safe variant of
// ComputeAttackPathMitreBoost. Runtime events and the authoritative Pod join are
// constrained by both cluster_id and pod_uid.
func ComputeAttackPathMitreBoostForIdentity(db *gorm.DB, id resourceidentity.Identity, window time.Duration, pathRelevantMitre map[string]struct{}) (boost float64, correlationPrecision float64, meta map[string]interface{}) {
	meta = map[string]interface{}{}
	if db == nil || id.Validate() != nil || !db.Migrator().HasTable("runtime_events") {
		return 0, 1.0, meta
	}
	var targetNS string
	_ = db.WithContext(context.Background()).Raw(
		`SELECT COALESCE(namespace,'') FROM pods WHERE cluster_id = ? AND uid = ? AND deleted_at IS NULL LIMIT 1`,
		id.ClusterID, id.ResourceUID,
	).Scan(&targetNS)
...
INNER JOIN pods AS p
  ON p.cluster_id = re.cluster_id
 AND p.uid = re.pod_uid
...
ORDER BY re.created_at DESC`
	if err := db.WithContext(context.Background()).Raw(q, id.ClusterID, id.ResourceUID, since).Scan(&rows).Error; err != nil {
		return 0, 1.0, meta
	}
	n := len(rows)
	if n == 0 {
	...
	boost = math.Min(MitreBoostCap(), burst*MitreBoostScale())
