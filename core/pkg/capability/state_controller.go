...
func (csc *CapabilityStateController) PromoteCapability(ctx context.Context, podUID, capabilityID, signalType string, signalConfidence float64) error {
	...
	err := csc.db.WithContext(ctx).Transaction(func(txDB *gorm.DB) error {
		didPromote, err := csc.promoteCapabilityTx(ctx, txDB, podUID, capabilityID, signalType, signalConfidence)
		if err != nil {
			return err
		}
	...
	// Layer 3 orchestration: recompute attack paths after successful promotion so
	// downstream path-based risk dimensions are not stale.
	if promoted {
		ScheduleAttackPathRebuild(csc.db, podUID)
	}
	return nil
}

// promoteCapabilityTx performs the actual promotion logic within a transaction.
func (csc *CapabilityStateController) promoteCapabilityTx(ctx context.Context, txDB *gorm.DB, podUID, capabilityID, signalType string, signalConfidence float64) (bool, error) {
