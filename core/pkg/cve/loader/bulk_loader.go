...
func (l *BulkLoader) createTempCVETable(ctx context.Context) error {
	...
	CREATE TEMPORARY TABLE IF NOT EXISTS cves_temp (
		...
		catalog_generation_id BIGINT NOT NULL DEFAULT 0
	);
	`
...
func (l *BulkLoader) batchInsertCVEsToTemp(ctx context.Context, cves []*ParsedCVE) error {
	...
	args := make([]interface{}, 0, len(cves)*14)
	argIndex := 1

	for i, cve := range cves {
		// Cast cve_references to JSONB
		values[i] = fmt.Sprintf("($%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d::jsonb, $%d, $%d, $%d)",
	...
	sql := fmt.Sprintf(`
		INSERT INTO cves_temp (
			...
			exploit_sources, cve_references, cwe_ids, source, catalog_generation_id
		) VALUES %s
	`, strings.Join(values, ","))
