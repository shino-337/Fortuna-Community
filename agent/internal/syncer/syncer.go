...
import (
	...
	agenttypes "github.com/fortuna/agent/pkg/types"
)
...
type SyncData struct {
	...
	Deployments         []agenttypes.DeploymentData `json:"deployments"`
	ReplicaSets         []agenttypes.ReplicaSetData `json:"replicasets"`
}
...
func (s *Syncer) buildPayload(ctx context.Context) (*SyncPayload, error) {
	...
	deploymentPayloads := make([]agenttypes.DeploymentData, 0, len(deployments.Items))
	for i := range deployments.Items {
		deploymentPayloads = append(deploymentPayloads, agenttypes.ConvertDeployment(&deployments.Items[i]))
	}

	replicaSetPayloads := make([]agenttypes.ReplicaSetData, 0, len(replicaSets.Items))
	for i := range replicaSets.Items {
		replicaSetPayloads = append(replicaSetPayloads, agenttypes.ConvertReplicaSet(&replicaSets.Items[i]))
	}
	...
	data := SyncData{
		...
		Deployments:         deploymentPayloads,
		ReplicaSets:         replicaSetPayloads,
	}
